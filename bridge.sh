...
# Sepolia / EVM 相关
ETH_RPC_URL="${ETH_RPC_URL:-https://sepolia.infura.io/v3/9a015995a5454ee089d24cfcf9a85d4f}"
...
ensure_tools() {
  ...
  # injectived / jq 为硬依赖，必须已有
  for cmd in injectived jq; do
...
ensure_injective_key_for_eth() {
  # 默认 key 名称可通过环境变量覆盖
  local key_name
  key_name="${INJ_ETH_KEY_NAME:-testOne}"
...
claim_inj() {
  ...
  injectived tx bank send genesis "$to_addr" "$coin" \
    --chain-id "$INJ_CHAIN_ID" \
    --keyring-backend file \
    --node "$INJ_NODE" \
    --gas auto \
